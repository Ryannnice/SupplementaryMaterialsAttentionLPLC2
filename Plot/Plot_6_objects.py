...
# 视频路径
video_path = r'C:\A_RESEARCH\Off_Line\C\LPLC2\synthetic_vedios\Vedio_dynamic_complex_bg\outdoor_3_black_expand_multi_6.avi'
...
# 文件路径和名称
file_name = 'outdoor_3_black_expand_multi_6_Fatal.txt'
...
offset = 0  # 偏移量设为0，以确保脚注位于图像正下方
# 在图像下方添加 Frame 信息
for i, num in enumerate(frame_numbers[:9]):  # 只为前八幅图像添加标注
    ax0.text(
        (i + 0.5) * concatenated_image.shape[1] // 9,  # 计算每个子图的中心位置
        concatenated_image.shape[0] + 15,  # 将 y 坐标设置为图像下方，15为垂直偏移值，可根据需要调整
        f'Frame {num+i}',  # 标注帧数
        color='black', 
        fontsize=8, 
        va='top',  # 设置文字的垂直对齐为顶部
...
for idx, line in enumerate(lines):
    ...
    # 绘制折线
    ax1.plot(x_values, y_values_normalized, label=f'AF {idx + 1}:({x_annotation},{y_annotation}), {data[3]}~{data[4]} frames', linewidth=1)
...
# 添加图例，放在左上角
ax1.legend(title="Center and Duration of AFs", loc='upper left',fontsize=8)

# 设置整体布局，紧贴子图
plt.tight_layout(h_pad=0)

# 保存图形
save_path = r'C:\\A_RESEARCH\\Off_Line\\C\\LPLC2\\Output_Plot\\' + '6_objects.png'
