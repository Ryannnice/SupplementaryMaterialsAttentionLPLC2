...
# 文件名和路径
file_name_0 = 'outdoor_2_black_4_period_LGMD2.txt'
...
# 读取数据
data_0 = np.loadtxt(file_path + file_name_0, skiprows=4, encoding='gbk')
...
# 获取横坐标
x_values_0 = np.arange(len(data_0))
...
# 归一化
data_0_normalized = data_0 / np.max(data_0)
...
gs = gridspec.GridSpec(5, 1, height_ratios=[0.7, 1, 1, 1, 1])  # 设置每个子图的高度比例
...
ax0.plot(x_values_0, data_0_normalized, color='black', linewidth=1.5, label='LGMD2')
...
ax3.set_xticklabels(x_values_3[::50], fontsize=12 ) 
...
# 保存图形，按原比例
save_path = r'C:\\A_RESEARCH\\Off_Line\\C\\LPLC2\\Output_Plot\\' + '4_period_final_2.png'
