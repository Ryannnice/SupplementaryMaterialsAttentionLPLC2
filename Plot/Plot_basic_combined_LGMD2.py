...
# 实验类型 (添加 LGMD2)
experiment_types = ["LGMD2", "LPLC2", "aLPLC2", "mLPLC2"]

# colors for dirrerent models
colors = ['#4b4453','#008080', '#845ec2', '#ff8066']  

# 获取所有文件的最大值用于归一化
global_max = 0
mLPLC2_max = 0  # 用于存储 mLPLC2 的最大值
lgmd2_max = 0  # 用于存储 LGMD2 的最大值

# 首先计算 LPLC2, aLPLC2, 和 LGMD2 的最大值
for experiment_type in ["LPLC2", "aLPLC2", "LGMD2"]:
    for test_type in test_types:
        ...
        try:
            ...
            if experiment_type == "LGMD2" and max_value > lgmd2_max:
                lgmd2_max = max_value
...
# 创建子图，调整子图顺序 (包括 LGMD2)
fig, axs = plt.subplots(len(experiment_types) + 1, len(test_types), figsize=(9, 8))
    ...
    for row_idx, experiment_type in enumerate(experiment_types):
        ...
        try:
            if experiment_type == "mLPLC2":
                ...
                data = np.loadtxt(file_path, skiprows=4, encoding='gbk')
                if experiment_type == "LGMD2":
                    y_values = data / lgmd2_max if lgmd2_max != 0 else data
                else:
        ...
        if col_idx == len(test_types) - 1:
            labels = ['LGMD2', 'LPLC2', 'aLPLC2', 'mLPLC2']
            ax.legend(labels[row_idx:row_idx+1], loc='center right')
...
# 设置横坐标刻度和对应的标签
ax_bottom_left.set_xticks(np.arange(0, 51, 20))  # 设置横坐标刻度
ax_bottom_left.set_xticklabels(np.arange(0, 51, 20), fontsize=10)  # 设置横坐标刻度的字体大小


ax_bottom_left.set_yticks(np.linspace(-0.1, 1.1, 3) )  # 设置纵坐标刻度
ax_bottom_left.set_yticklabels(np.round(np.linspace(-0.1, 1.1, 3), 2), fontsize=10)  # 设置纵坐标刻度的字体大小

ax_bottom_left.set_xlabel("Time in Frames", fontsize=17)  # 横坐标标签，设置字体大小
# 设置纵坐标标签及其位置
ax_bottom_left.set_ylabel("Normalized Membrane Potential", fontsize=17, labelpad=5)  # labelpad调整标签与坐标轴的距离

ax_bottom_left.xaxis.set_label_coords(0.76, -0.2)  # 例如将标签移动到左侧并居中
ax_bottom_left.yaxis.set_label_coords(-0.267, 1.45)  # 例如将标签移动到左侧并居中

# 保存图形
save_path = os.path.join(output_folder_path, 'basic_combined_2.png')
